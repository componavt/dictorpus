...
trait TextWordBlock
{
    ...
    public function editWordBlock($word, $s_id, $search_word = null, $with_edit = null, $search_w = [], $meanings = [], $gramsets = [], $wordforms = [], $words_with_important_examples = [], $preloaded = false)
    {
        ...
        if (
            $search_word && Grammatic::changeLetters((string)$word, $this->lang_id) == $search_word
    ...
    {
        if (empty($meanings['checked']) && empty($meanings['unchecked'])) {
            return null;
        }
    ...
    public function buildUncheckedMeaningsNode(\SimpleXMLElement $parent, $unchecked_meanings, $s_word = '')
    {
        ...
        foreach ($unchecked_meanings as $meaning) {
            ...
            if (User::checkAccess('corpus.edit')) {
                ...
                $span->addAttribute('title', trans('corpus.mark_right_meaning'));
        ...
        if (User::checkAccess('corpus.edit')) {
            ...
            $input->addAttribute('value', trans('corpus.new_meaning'));
            $input->addAttribute(
                'onclick',
    ...
    public function buildGramsetBlock(\SimpleXMLElement $parent, $w_id, $gramsets, $wordforms)
    {
        if (!empty($gramsets['checked'])) {
            ...
            foreach ($gramsets['unchecked'] as $gramset) {
                ...
                $span->addAttribute('title', trans('corpus.mark_right_gramset') . ' (' . ($wordforms[$gramset->wordform_id] ?? '') . ')');
    ...
    public function createLemmaBlock($w_id)
    {
        ...
        if (!$meaning_checked && !sizeof($meaning_unchecked)) {
            return null;
        }
        ...
        if ($wordform) {
            ...
            foreach ($wordforms as $wordform) {
                ...
                $str .= '<p>' . Gramset::getStringByID($gramset_id)
                    ...
                    . '" class="fa fa-plus choose-gramset" title="' . trans('corpus.mark_right_gramset') . ' ('
