...
trait TextSelect
{
    ...
    /**
     * возвращает массив предложений с переводами, первым элементом - заголовки текста, диалекты добавляются по необходимости
     * [<text_title>=>['corpus'=><corpus_names>, 'dialect'=><dialect_names>, 'trans'=><transtext_title>]]
     * @return array
     */
    public function sentencesWithTranslation($sentences, $without_sentences = [], $with_dialect = false)
    {
        ...
        $dialects = $with_dialect ? $this->dialectsToString() : '';
        ...
        $sents = Sentence::whereTextId($this->id)
...
if (empty($trans_sentences[$sentence->s_id])) {
    ...
    /**
     * возвращает массив предложений без переводов, первым элементом - заголовки текста;
     * [<text>=>['corpus'=><corpus_names>, 'dialect'=><dialect_names>]]
     * @return array
     */
    public function sentencesWithoutTranslation($sentences, $without_sentences = [])
    {
        $corpuses = $this->corpusesToString();
        $dialects = $this->dialectsToString();
        $sentences[$this->title] = [
            'corpus' => $corpuses,
            'dialect' => $dialects,
        ];
    ...
    {
        $label_id = 3; // метка Lipas
        $sids = [];
        $sentence_ids = []; // id собранных предложений, чтобы потом исключить из множества

        // ищем значения лемм, включенных в Lipas
        ...
        foreach ($meanings as $meaning) {
            // выбираем те предложения-примеры, которые получили оценку 10, признак включения в Lipas
        ...
        foreach ($sids as $text_id => $sents) {
            ...
            foreach ($sents as $s_id => $w_ids) {
                ...
                foreach ($w_ids as $w_id) {
                    ...
                    // если есть фрагмент, то берем его, если нет, то исходный текст предложения
                    ...
                    // если есть отдельный перевод предложения, берем его, если нет, то из полного перевода текста
