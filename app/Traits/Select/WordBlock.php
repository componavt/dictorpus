...
trait WordBlock
{
    ...
    public static function createWordBlock($text_id, $w_id)
    {
        ...
        if (!$word_obj->checkedMeaning() && !sizeof($word_obj->uncheckedMeanings())) {
            return null;
        }
        ...
        foreach ($this->uncheckedMeanings() as $meaning) {
            ...
            if (User::checkAccess('corpus.edit')) {
                $str .= '<span class="fa fa-plus choose-meaning" data-add="'
                    ...
                    . trans('corpus.mark_right_meaning') . '" onClick="addWordMeaning(this)"></span></p>';
            }
        }
        if (User::checkAccess('corpus.edit')) {
            $str .= "<input class=\"add-wordform-link\" type=\"button\" value=\""
                . trans('corpus.new_meaning') . "\" onClick=\"callAddWordform(this, '"
