...
trait TextExportExcel
{
    public function annotatedTextExport($dir, $type = 1)
    {
        ...
        foreach ($sentences as $sentence) {
            ...
            $words_from_xml = $sentence->getWordsFromXML();
            list($sheet, $row) =
                $this->{'annotatedTextExportSentence' . ($type == 3 ? '3' : '')}(
                    $sheet,
                    $row,
                    trim(strip_tags(preg_replace('/[¦^]/u', '', $sentence->text_xml))),
                    strip_tags(preg_replace('/[¦^]/u', '', $cyr_sentences[$sentence->s_id]['sentence'])),
                    strip_tags(preg_replace('/[¦^]/u', '', $trans_sentences[$sentence->s_id]['sentence'] ?? ''))
                );
            list($sheet, $row) =
                $this->{'annotatedTextExportWords' . ($type == 3 ? '3' : '')}(
                    $sheet,
                    $words,
                    $words_from_xml,
                    $cyr_words,
...
*/
    ...
    public function annotatedTextExportWords($sheet, $words, $words_from_xml, $cyr_words, $row, $type = 1)
    {
        $out = [];
        $i = 1;
        foreach ($words as $word) {
            ...
            $out[4][$letter] = $words_from_xml[$word->w_id];
            $i++;
        }
    ...
    public function annotatedTextExportWords3($sheet, $words, $words_from_xml, $cyr_words, $row)
    {
        $i = 1;
        foreach ($words as $word) {
            $gramset = $word->checkedGramset();
            $meaning = $word->checkedMeaning();
            $word_normal = $words_from_xml[$word->w_id];
            $sheet = $this->sheetCellSet($sheet, 'A' . $row, empty($cyr_words[$word->w_id]) ? '' : $cyr_words[$word->w_id]);
            $sheet = $this->sheetCellSet($sheet, 'B' . $row, $i == 1 ? mb_strtolower($word_normal) : $word_normal);
