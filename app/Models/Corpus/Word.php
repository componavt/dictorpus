...
class Word extends Model
{
    ...
    public function isChangeable()
    {
        ...
        return (bool)DB::table('gramset_pos')->where('lang_id', $lang)
    ...
    public static function search(array $url_args)
    {
        ...
        $words = self::searchByLang($words, $url_args['search_lang']);
    ...
    public static function searchByLang($texts, $lang_id)
    {
        ...
        return $texts->whereIn('text_id', function ($query) use ($lang_id) {
    ...
    public function mergeWords($words_to_merge)
    {
        ...
        if (!$sxe || $error_message) {
            return $error_message;
        }
        //dd($sxe);        
        $last_node = $sxe->xpath("//w[@id='" . $last_id . "']");
        if (!$this->checkWord((string)$last_node[0])) {
            return;
        }
    ...
    public function moveCharOut($char)
    {
        ...
        if (
            preg_match("/^" . $char . "(.+)$/", $this->word, $w_parts) &&
