...
class Concept extends Model implements HasMediaConversions
{
    ...
    public function photoInfo()
    {
        ...
        $thumbPath = $this->getFirstMediaPath('images', 'thumb');
        if (!$thumbPath) {
            $thumbPath = $this->getFirstMediaPath('images');
        }

        return [
            'id' => $this->getFirstMedia('images')->id,
            'url' => self::WIKI_URL . $this->wiki_photo_encoded,
            'source' => $local_src,
            'thumb_path' => $thumbPath ?: null,
            'wiki_photo' => $this->wiki_photo
        ];
    }
    ...
    public function photoPreview()
    {
        ...
        if ($this->src) {
            return [
                'url' => self::WIKI_URL . preg_replace("/\s/", "_", $this->wiki_photo),
        ...
        $url = $this->photoInfo();
        return [
            'url' => self::WIKI_URL . preg_replace("/\s/", "_", $this->wiki_photo), //isset($url['url']) ? $url['url'] : null,
