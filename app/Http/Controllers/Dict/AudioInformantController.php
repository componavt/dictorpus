...
class AudioInformantController extends Controller
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->middleware('auth:dict.audio,/dict/audio');
    ...
    public function destroy($informant_id, $id)
    {
        ...
        if ($id != "" && $id > 0) {
            try {
                $audio = Audio::find($id);
                if ($audio) {
                    $result['message'] = trans('dict.audio_removed', ['name' => $audio->filename]);
