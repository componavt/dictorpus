...
class LemmaWordformController extends Controller
{
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if ($id > 0) {
            try {
                ...
                $lemma = Lemma::findOrFail($id);
                if ($lemma) {
                    $result['message'] = trans('dict.wordforms_removed');
                    $lemma->wordforms->wherePivot('dialect_id', $dialect_id)->detach();
                } else {
                    $error = true;
    ...
    public function load($id, Request $request)
    {
        ...
        return view(
            'dict.lemma_wordform._wordform_table',
    ...
    public function affixFrequency(Request $request)
    {
        ...
        $lemmas_for_lang = Lemma::join('lemma_wordform', 'lemmas.id', '=', 'lemma_wordform.lemma_id')
        ...
        $lemmas_for_pos = Lemma::join('lemma_wordform', 'lemmas.id', '=', 'lemma_wordform.lemma_id')
            ->whereNotNull('gramset_id')
            ->whereNotNull('affix')
            ->groupBy('pos_id')
            ->latest('frequency')
        ...
        if ($url_args['search_lang']) {
            ...
            $lemmas = $lemmas
                ...
                //                    ->take(1000)
                ->get(['pos_id', 'gramset_id', 'affix', DB::raw('REVERSE(affix) as reverse_affix'), DB::raw('count(*) as frequency')]);
        ...
        return view(
            'dict.lemma_wordform.affix_freq',
    ...
    public function posCommonWordforms()
    {
        ...
        for ($i = 0; $i < sizeof($parts_of_speech) - 1; $i++) {
            for ($j = $i + 1; $j < sizeof($parts_of_speech); $j++) {
                $common = array_intersect(
                    array_keys($pos_wordforms[$parts_of_speech[$i]]),
        ...
        return view(
            'dict.lemma_wordform.pos_common_wordforms',
            compact(
                'search_lang',
                'wordforms_total',
                'unique_wordforms_total',
                'common_wordforms_total',
                'common_wordforms_total_counts',
