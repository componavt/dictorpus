...
class CorrectController extends Controller
{
    ...
    function addWordformAffixes(Request $request)
    {
        ...
        foreach (Lang::projectLangIDs() as $l_id) {
            ...
            $langs[$l_id]['affix_count'] = number_format((float)Wordform::countWithoutAffixes($l_id), 0, ',', ' ');
    ...
    public function checkParallelTexts()
    {
        ...
        foreach ($texts as $text) {
            ...
            //dd($sxe);            
            if ($error_message) {
                return 'Text XML parsing for ' . $text->id . ': ' . $error_message;
            }

            $trans_text = Transtext::find($text->transtext_id);
            if (!$trans_text) {
                return 'Transtext finding error.';
            }

            list($sxe_trans, $error_message) = Text::toXML($trans_text->text_xml, $trans_text->id);
            if ($error_message) {
                return 'Transtext XML parsing for ' . $text->id . ': ' . $error_message;
            }
    ...
    public function generateWordforms(Request $request)
    {
        ...
        Correct::generateWordforms(
            $lang_id,
            $request->input('search_pos'),
            (int)$request->input('w_count')
        );
    }
