...
//print "<p>".$lemma->id."</p>";            
    ...
    public function updateLemma($id, Request $request)
    {
        ...
        DB::transaction(function () use ($lemma, $data) {
            ...
            $lemma->updateLemma($data, false);
            $lemma->updateTextLinks(false);

            $after = $lemma->textWordformPairKeys();

            foreach ($before + $after as $pair) {
                Wordform::reconcileTextWordformLinksForPair($pair[0], $pair[1]);
            }
        });
    ...
    public function storePhrase($meaning_id, Request $request)
    {
        ...
        if (!$lemma_p) {
            $lemma_p = Lemma::store(
                $request->phrase,
                PartOfSpeech::PhraseId,
                $meaning->lemma->lang_id
            );
    ...
    public function editPhrase($id)
    {
        ...
        return view(
            'service.dict.lemma._create_edit_phrase',
