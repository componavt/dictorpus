    ...
    public function illativeTable()
    {
        ...
        foreach ($parts_of_speech as $pos_id) {
            ...
            foreach ($lemma_coll as $lemma):
                ...
                if (
                    $lemma_wordforms['ill_sg']['new'] != $lemma_wordforms['ill_sg']['old'] ||
    ...
    public function tmpUpdateStemAffix()
    {
        ...
        foreach ($lemmas as $lemma) {
            ...
            if ($max_stem != $lemma->reverseLemma->stem || $affix != $lemma->reverseLemma->affix || !sizeof($stems)) {
                print sprintf(
                    "<p><b>id:</b> %s, <b>lang:</b> %s, <b>lemma:</b> <a href=\"/dict/lemma/%s\">%s</a>, <b>dialects:</b> [%s], <b>stems:</b> [%s], <b>max_stem:</b> %s, <b>affix:</b> %s",
                    $lemma->id,
                    $lemma->lang_id,
                    $lemma->id,
                    $lemma->lemma,
