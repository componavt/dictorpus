...
class KarName
{
    ...
    public static function stem5FromMiniTemplate($stem0, $stem1, $stem6, $lang_id, $pos_id, $harmony, $stem0_syll, $dialect_id)
    {
        ...
        if ($dialect_id == 47 && preg_match("/" . $C . "’?[iuyoö]$/u", $stem6)) {
            ...
            if (($stem6_syll == 2 && preg_match("/^" . $C . "*" . $V . "*[uo]/u", $stem6)) // А.4.1.1
                || ($stem6_syll > 2 && (preg_match("/m[pb]?a$/u", $stem6) || $pos_id == 1 && preg_match("/va$/u", $stem6)))
            ) { // А.4.1.2
                return $regs[1] . 'i';
            ...
            if (
                $stem6_syll == 2 // А.5.1.1
                || $stem6_syll > 2 && preg_match("/[mvžsjpb]ä$/u", $stem6)
            ) { // А.5.1.2
                return $regs[1] . 'i';
    ...
    public static function stem4FromMiniTemplate($stem1, $stem5, $stem6, $harmony, $dialect_id)
    {
        ...
        if (
            preg_match("/" . $C . "[oö]i$/u", $stem5) && preg_match("/" . $C . "[aä]$/u", $stem6)
            && $syl_count5 == $syl_count6
        ) {                      // А
            ...
            $dialect_id != 47 && preg_match("/" . $C . "[oöuy]i$/u", $stem5)
            && preg_match("/" . $C . "[oöuy]$/u", $stem6) && $syl_count5 == $syl_count6
        ) {         // Б
            return $stem1 . 'i';
        } elseif (
            $syl_count5 > $syl_count6 && ($dialect_id == 47 && preg_match("/" . $C . "[oö]i$/u", $stem5)
    ...
    public static function suggestTemplates($lang_id, $word)
    {
        ...
        if (
            preg_match('/' . $C . '’[aä]$/u', $word)                        // hil’l’a
            || preg_match('/[uy]{2}$/u', $word)
        ) {                // alušpuu     
            $templates[] = $word . ' [, ]';
        }
        if (
            preg_match('/' . $C . '’?[aäiuyoö]$/u', $word)                    // šyvä
            || preg_match('/' . $V . $V . '$/u', $word)
        ) {                  // työ     
            $templates[] = $word . ' []';
        }
        ...
        if (
            preg_match('/^(.+?)([lnrsš])(t)([aäoöuyi])$/u', $word, $r)  // kulta
            || preg_match('/^(.+?)(m)(p)([aäoöuyi])$/u', $word, $r)
        ) { // lämpö
        ...
        if (
            preg_match('/^(.+?e)(š)$/u', $word, $r)                      // mieš
            || preg_match('/^(.+?i)(s)$/u', $word, $r)
        ) {         // kiärmis
        ...
        if (
            preg_match('/^(.+?' . $V . $C . ')(e)$/u', $word, $r)         // iltakoite
            || preg_match('/^(.+?)(eki)$/u', $word, $r)              // reki
            || preg_match('/^(.+?)(et)$/u', $word, $r)
        ) {         // apposet
        ...
        if (
            preg_match('/^(.+?)(jeh)$/u', $word, $r)                    // aijeh
            || preg_match('/^(.+?[lk])(eh)$/u', $word, $r)         // eläkeh
            || preg_match('/^(.*?r[uy])(is)$/u', $word, $r)
        ) {       // ruis
            $templates[] = $r[1] . '|' . $r[2] . ' [kehe, ' . $r[2] . ']';
        }
        if (
            preg_match('/^(.+?š)(e)(l)$/u', $word, $r)                   // ašel
            || preg_match('/^(.*?i)(je)(n)$/u', $word, $r)
        ) {        // ijen
        ...
        if (
            preg_match('/^(.+?m)(p[aä])$/u', $word, $r)                 // kampa
            || preg_match('/^(.+?m)([bp]i)$/u', $word, $r)
        ) {    // nuorembi
            $templates[] = $r[1] . '|' . $r[2] . ' [m' . $A . ']';
        }
        if (
            preg_match('/^(.+?[aäi])(n)$/u', $word, $r)                // kaššin
            || preg_match('/^(.+?V)(in)$/u', $word, $r)
        ) {        // huokain
        ...
        if (
            preg_match('/^(.+?r[uy])([sš])$/u', $word, $r)            // ankaruš
            || preg_match('/^(.+?[uy])(t)$/u', $word, $r)
        ) {      // katonut
        ...
        if (
            preg_match('/^(.+?m)(m[aä]š)$/u', $word, $r)               // hammaš
            || preg_match('/^(.+?p)([aä]š)$/u', $word, $r)      // jiäruopaš
            || preg_match('/^(.+?)(v[aä]š)$/u', $word, $r)
        ) {      // karvaš
        ...
        if (
            preg_match('/^(.+?)(veh)$/u', $word, $r)                   // helveh
            || preg_match('/^(.+?)(vis)$/u', $word, $r)
        ) {         // tarvis
            $templates[] = $r[1] . '|' . $r[2] . ' [pehe, ' . $r[2] . ']';
        }
        if (
            preg_match('/^(.+?m)(me)(l)$/u', $word, $r)                // vemmel
            || preg_match('/^(.+?)(ve)(n)$/u', $word, $r)
        ) {        // haven
        ...
        if (
            preg_match('/^(.+?[ht])([aä]š)$/u', $word, $r)              // puhaš
            || preg_match('/^(.+?)(j[aä]š)$/u', $word, $r)
        ) {       // hijaš
        ...
        if (
            preg_match('/^(.+?)([hj]e)(h)$/u', $word, $r)               // kajeh
            || preg_match('/^(.+?n)(ne)(l)$/u', $word, $r)         // kannel
            || preg_match('/^(.+?n)(ne)(r)$/u', $word, $r)
        ) {      // manner
        ...
        if (
            preg_match('/^(.+?)([aä]k[aä])$/u', $word, $r)               // haka
            || preg_match('/^(.+?)([aä]t[aä])$/u', $word, $r)
        ) {  // holvata
        ...
        if (
            preg_match('/^(.+?[ht])(tu)([aä])$/u', $word, $r)      // juokšuttua
            || preg_match('/^(.+?' . $C . ')([uy])([aä])$/u', $word, $r)
        ) { // čirškua
            $templates[] = $r[1] . '|' . $r[2] . $r[3] . ' [' . $r[3] . ', ' . $r[2] . $r[3] . ']';
        }
        if (
            preg_match('/^(.+?[thlrš])(k)([aäoöuy])$/u', $word, $r)     // pitkä
            || preg_match('/^(.+?[uyhr])(t)([aä])$/u', $word, $r)
        ) { // parta
