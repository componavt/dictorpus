...
<ol>
    @for ($i=1; $i<3; $i++)
    <li>в форме номинатива ед.ч. заканчиваются на {{$u[$i]}} или {{$a[$i]}}, при этом в форме генитива ед.ч. заканчиваются на {{$a[$i]}}n</li>
    <ul>
    ...
    @endfor
    @for ($i=1; $i<3; $i++)
    <li>в форме номинатива ед.ч. заканчиваются на согласную, при этом в форме генитива ед.ч. заканчиваются на {{$a[$i]}}n</li>
    <ul>
        @foreach ($pos_list as $pos_code => $pos_name)
        <li>{{$pos_name}}</li>
        <ul>
